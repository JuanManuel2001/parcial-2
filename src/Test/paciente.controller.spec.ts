...
import { PacienteController } from '../Entidades/Pacientes/paciente.controller';
import { PacienteService } from '../Entidades/Pacientes/paciente.service';
...
describe('PacienteController', () => {
  ...
  const mockPacienteService = {
    create: jest.fn((paciente) => {
      // Modificación aquí: en lugar de `throw`, usamos `Promise.reject()` para rechazar la promesa
      if (paciente.nombre.length < 3) {
        return Promise.reject(new Error('El nombre del paciente debe tener al menos 3 caracteres.'));
      }
  ...
  describe('create', () => {
    ...
    it('Debe lanzar una excepción si el nombre tiene menos de 3 caracteres', async () => {
      ...
      // Usamos `rejects.toThrow` para verificar que la promesa se rechace con el error adecuado
